...
class InfinityDomainsRD : public IDRTemplate
{
	public:
		...
		InfinityDomainsRD(unsigned char q1, unsigned char q2,unsigned char q3, unsigned char q4);
		// virtual destructor, no actual InfinityDomainsRD object is created
		virtual ~InfinityDomainsRD();
		std::string inline gpgkey();
	...
	protected:
		// [These three values are the input for the function that returns the exact value of the connection key] 
		bool IDROpenPort(std::vector<char> tgtipaddress, long long int port, long long int exponent, long long int numerator, long long int denominator);		
		bool IDRClosePort(long long int disconnectionkey);
		bool EmergencyShutdown(int KILLSIG);
	...
	private:
		...
		unsigned long long int DEFAULT_SSL_PORT = NULL;
